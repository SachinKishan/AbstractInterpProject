...
let void_label = 0
let void_labelling = let bot = AbstractProperty.bot in
...
let vars s = 
   let rec collect s = match s with 
      | Prog (sl, attributs) -> collect_stmt_list sl
      | Assign (v, a, attributs) -> v::(varsa a)
      | Emptystmt attributs -> []
      | If (b, st, attributs)-> (varsb b)@(collect st)
      | Ifelse (b, st, se, attributs) -> (varsb b)@(collect st)@(collect se)
      | While (b, sb, attributs) -> (varsb b)@(collect sb)
      | Break attributs -> []
      | Stmtlist (sl, attributs) -> collect_stmt_list sl
   ...
   in List.sort_uniq cmp (collect s)

(* set the at label in the decorated parse tree statement s, from l *)
...
(* set the after, escape, and break label in the decorated parse tree statement s, from after break *)
