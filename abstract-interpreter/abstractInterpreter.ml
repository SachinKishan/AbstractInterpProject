...
let rec lfp_widen a f leq widen =
  let fa = widen a (f a) in
    if leq fa a then fa else lfp_widen fa f leq widen

let rec lfp_narrow a f leq narrow =
  let fa = narrow a (f a) in
    if leq a fa then fa else lfp_narrow fa f leq narrow 

let widen_while x y = match x, y with
   | (While (b, sb, (at,atP,af,afP,es,br,brP))),
        (While (b', sb', (at',atP',af',afP',es',br',brP'))) ->
           While (b, sb, (at, widen atP atP', af, widen afP afP', es, br, widen brP brP'))
   | _,_ -> failwith "abstractInterpreter: incorrect widen_while argument"

let narrow_while x y = match x, y with
   | (While (b, sb, (at,atP,af,afP,es,br,brP))),
        (While (b', sb', (at',atP',af',afP',es',br',brP'))) ->
           While (b, sb, (at, narrow atP atP', af, narrow afP afP', es, br, narrow brP brP'))
   | _,_ -> failwith "abstractInterpreter: incorrect narrow_while argument"
...
and abstractInterpreter s r0 = match s with 
   ...
   | While (b, sb, (at,atP,af,afP,es,br,brP)) -> 
     let wbot = (While (b, sb, (at,r0,af,bot,es,br,bot))) in 
        let w = lfp_widen wbot (fWhile r0) leq widen_while in
           let (While (b, sb, (at,atP,af,afP,es,br,brP))) = 
                lfp_narrow w (fWhile r0) leq narrow_while in
                   let sb' = abstractInterpreter sb (test b atP) in 
                       While (b, sb', (at,atP,af,afP,es,br,brP))
